  ...
  {#if data.position}
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
      <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-4">
        ...
        <p class="text-lg font-semibold money-mask">{num(data.position.quantity)}</p>
      </div>
      <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-4">
        <p class="text-xs text-gray-500 mb-1">Avg cost</p>
        <p class="text-lg font-semibold"><Money value={data.position.average_cost} /></p>
      </div>
      <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-4">
        <p class="text-xs text-gray-500 mb-1">Market value</p>
        <p class="text-lg font-semibold"><Money value={data.position.market_value} /></p>
      </div>
      <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-4">
        <p class="text-xs text-gray-500 mb-1">Unrealized P&L</p>
        <p class="text-lg font-semibold {pctColor(data.position.unrealized_pnl)}">
          <Money value={data.position.unrealized_pnl} /> ({pct(data.position.unrealized_pnl_pct)})
        </p>
      </div>
    </div>
      ...
      <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-4">
        {#each data.position.dividend_receipts as d}
          <div class="flex items-center justify-between py-1.5 text-sm">
            ...
            <span class="text-gray-500"><Money value={d.total_amount} /></span>
          </div>
        {/each}
      </div>
    {/if}
