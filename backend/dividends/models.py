...
class DividendReceipt(models.Model):
    """
    Computed per-portfolio record: that portfolio's held quantity as of
    record_date x amount_per_share. Generated by a service function when a
    DividendRecord is created/updated — not user-editable.
    """

    portfolio = models.ForeignKey(
        "portfolio.Portfolio", on_delete=models.CASCADE, related_name="dividend_receipts"
    )
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(fields=["portfolio", "dividend_record"], name="uniq_receipt_per_portfolio_dividend")
        ]

    def __str__(self):
        return f"{self.portfolio} — {self.dividend_record}: GHS {self.total_amount}"
